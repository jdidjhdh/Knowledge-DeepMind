...
class MemoryService:
    ...
    def delete_user_memory(self, user_id: str) -> int:
        result = self.db.execute("DELETE FROM memory_items WHERE user_id = ?", (user_id,))
    ...
    def get_stats(self) -> dict:
        user_count = self.db.execute("SELECT COUNT(*) as cnt FROM user_profiles").fetchone()["cnt"]
        item_count = self.db.execute("SELECT COUNT(*) as cnt FROM memory_items WHERE weight >= 0.1").fetchone()["cnt"]
        conv_count = self.db.execute("SELECT COUNT(DISTINCT conv_id) as cnt FROM conversation_history").fetchone()["cnt"]
        msg_count = self.db.execute("SELECT COUNT(*) as cnt FROM conversation_history").fetchone()["cnt"]
        return {
